...
export const getCurrentPost = createRegistrySelector( ( select ) => ( state ) => {
	...
	// TODO: This exists for compatibility with the previous selector behavior
 ...
 * @return {*} Post attribute value.
 */
const getNestedEditedPostProperty = createRegistrySelector( ( select ) => ( state, attributeName ) => {
	const edits = getPostEdits( state );
...
export function getEditorBlocks( state ) {
	return getEditedPostAttribute( state, '_blocks' ) || EMPTY_ARRAY;
}
